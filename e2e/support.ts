...
export async function preparePage(
  ...
  await page.addInitScript(
    ({ curriculumKey, layoutKey, curriculumState, layout }) => {
      if (sessionStorage.getItem("__e2e_prepared__")) return;
      sessionStorage.setItem("__e2e_prepared__", "true");
...
import { ensureSpacedDrillItems } from "../src/lib/bengali-grapheme";

export function withInsertedSpaces(items: string[], interval = 3): string[] {
  return ensureSpacedDrillItems(items, interval);
}
...
export async function waitForLessonReady(page: Page) {
  await expect(page.getByText(/(?:অক্ষর|বাক্য)\s+১\s*\/\s*[\d\u09E6-\u09EF]+/)).toBeVisible();
