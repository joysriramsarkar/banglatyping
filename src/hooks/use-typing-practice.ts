...
function handleInitOrReset(state: TypingState, action: Extract<TypingAction, { type: 'INIT' | 'RESET' }>): TypingState {
...
function handleInputChar(state: TypingState, action: Extract<TypingAction, { type: 'INPUT_CHAR' }>): TypingState {
...
function handleSetInput(state: TypingState, action: Extract<TypingAction, { type: 'SET_INPUT' }>): TypingState {
  if (state.isFinished) return state;
...
function handleBackspace(state: TypingState): TypingState {
  ...
  if (currentInput.length > 0) {
    ...
    try {
      ...
      const segments = Array.from(segmenter.segment(currentInput)) as { segment: string }[];
...
function handleSpace(state: TypingState): TypingState {
  if (state.isFinished) return state;
  const currentInput = (state.charInputPerWord[state.currentWordIndex] || '').normalize('NFC');
...
function handleNavigate(state: TypingState, action: Extract<TypingAction, { type: 'NAVIGATE' }>): TypingState {
  if (state.isFinished) return state;
...
function handleCalculateStats(state: TypingState, action: Extract<TypingAction, { type: 'CALCULATE_STATS' }>): TypingState {
  if (state.isFinished) return state;
...
function typingReducer(state: TypingState, action: TypingAction): TypingState {
  switch (action.type) {
    case 'INIT':
    case 'RESET':
      return handleInitOrReset(state, action);
    case 'INPUT_CHAR':
      return handleInputChar(state, action);
    case 'SET_INPUT':
      return handleSetInput(state, action);
    case 'BACKSPACE':
    ...
    case 'CALCULATE_STATS':
      return handleCalculateStats(state, action);
    case 'FINISH':
