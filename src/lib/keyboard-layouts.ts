...
const avroLayout: KeyboardLayoutConfig = {
  ...
  top: [
    ...
    { key: 'y', keyCode: 'KeyY', bn: 'য়', bnShift: 'য', ...baseFinger(7, 'Index') },
  ...
  bottom: [
    ...
    { key: 'z', keyCode: 'KeyZ', bn: 'জ', bnShift: 'ঝ', ...baseFinger(1, 'Pinky') },
    { key: 'x', keyCode: 'KeyX', bn: 'চ', bnShift: 'ছ', ...baseFinger(2, 'Ring') },
    { key: 'c', keyCode: 'KeyC', bn: 'ক', bnShift: 'খ', ...baseFinger(3, 'Middle') },
    { key: 'v', keyCode: 'KeyV', bn: 'ব', bnShift: 'ভ', ...baseFinger(4, 'Index') },
    { key: 'b', keyCode: 'KeyB', bn: 'ন', bnShift: 'ণ', ...baseFinger(4, 'Index') },
    { key: 'n', keyCode: 'KeyN', bn: 'ম', bnShift: 'ং', ...baseFinger(7, 'Index') },
    { key: 'm', keyCode: 'KeyM', bn: 'া', bnShift: '।', ...baseFinger(7, 'Index') },
...
export function normalizeKeyboardLayout(value?: string): KeyboardLayoutKey {
  const normalized = (value || 'avro').toLowerCase();
  if (normalized === 'bijoy' || normalized === 'bijoy classic') return 'bijoy';
  if (normalized === 'banglaword' || normalized === 'banglaword') return 'banglaword';
  return 'avro';
}

export function getKeyboardLayoutConfig(layout?: string): KeyboardLayoutConfig {
  return layouts[normalizeKeyboardLayout(layout)];
}

export function getKeyboardLayoutOptions() {
  return Object.values(layouts).map((layout) => ({ value: layout.id, label: layout.label }));
}
